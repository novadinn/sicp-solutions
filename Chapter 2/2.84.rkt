...
(define type-tower
  (lambda (type)
    (cond
      ((equal? type 'scheme-number) 0)
      ((equal? type 'rational) 1)
      ((equal? type 'complex) 2)
      (else (error "Undefined type : TYPE-TABLE")))))
          ...
          (if (= (length args) 2)
              (let ((type1 (car type-tags))
                ...
                (let ((t1->t2 (get-coercion type1 type2))
                      ...
                      (ord1 (type-tower type1))
                      (ord2 (type-tower type2)))
                  (cond
                    ((= ord1 ord2) (error "No method for current types"))
                    ((< ord1 ord2) (apply-generic op (raise a1) a2))
